...
// ============================================================================
// V32: fp32 2x unroll + __ldg() + dynamic block
// - 2x loop unroll (less aggressive than v31's 4x, better at large D)
// - __ldg() for all weight/bias reads
// - block_size = 512 for D >= 4096, 256 otherwise
// ============================================================================

template<typename T>
__global__ void rmsnorm_v32_fp32_unroll_kernel(
    ...
    // 2x unrolled sum-of-squares
    int64_t tiles = hidden_dim / (blockDim.x * 2);
    int64_t tiled_end = tiles * blockDim.x * 2;

    float sum_sq0 = 0.0f, sum_sq1 = 0.0f;
    for (int64_t i = threadIdx.x; i < tiled_end; i += blockDim.x * 2) {
    ...
    float total = block_reduce_sum(sum_sq0 + sum_sq1, smem, blockDim.x);
    float rms = rsqrtf(total / hidden_dim + eps);

    // 2x unrolled normalize with __ldg()
    for (int64_t i = threadIdx.x; i < tiled_end; i += blockDim.x * 2) {
...
__global__ void rmsnorm_v32_vec_kernel(
    const T* __restrict__ input,
    T* __restrict__ output,
    const T* __restrict__ weight,
    const T* __restrict__ bias,
...
// Scalar fallback
template<typename T>
__global__ void rmsnorm_v32_scalar_kernel(
    const T* __restrict__ input,
    T* __restrict__ output,
    const T* __restrict__ weight,
    const T* __restrict__ bias,
...
void rmsnorm_v32_fp32_unroll_cuda(
    torch::Tensor output,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        input.scalar_type(), "rmsnorm_v32_fp32_unroll",
        [&]() {
            ...
            if (aligned) {
                rmsnorm_v32_vec_kernel<scalar_t, vw><<<batch_size, block_size, smem_size>>>(
                ...
                rmsnorm_v32_scalar_kernel<scalar_t><<<batch_size, block_size, smem_size>>>(
                    input.data_ptr<scalar_t>(),
                    output.data_ptr<scalar_t>(),
                    weight.data_ptr<scalar_t>(),
                    bias.data_ptr<scalar_t>(),
