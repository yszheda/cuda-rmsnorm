...
// - Fully unroll both sum-of-squares and normalize loops for tiny D
// - Avoids loop overhead for small hidden dimensions
// - Falls back to v15-style for larger D
// ============================================================================

template<typename T, int vec_width, int HIDDEN_DIM>
__global__ void rmsnorm_v38_unroll_small_kernel(
...
// Scalar fallback
template<typename T>
__global__ void rmsnorm_v38_scalar_kernel(
    const T* __restrict__ input,
    T* __restrict__ output,
    const T* __restrict__ weight,
    const T* __restrict__ bias,
...
void rmsnorm_v38_unroll_small_cuda(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        input.scalar_type(), "rmsnorm_v38_unroll_small",
        [&]() {
            ...
            if (aligned && hidden_dim <= 256 && hidden_dim % vw == 0) {
                if (hidden_dim == 128) {
                    rmsnorm_v38_unroll_small_kernel<scalar_t, vw, 128><<<batch_size, block_size, smem_size>>>(
                        input.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(),
                        weight.data_ptr<scalar_t>(), bias.data_ptr<scalar_t>(),
                        eps, use_affine);
                } else if (hidden_dim == 256) {
                    rmsnorm_v38_unroll_small_kernel<scalar_t, vw, 256><<<batch_size, block_size, smem_size>>>(
                        input.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(),
                        weight.data_ptr<scalar_t>(), bias.data_ptr<scalar_t>(),
                        eps, use_affine);
                } else {
                    rmsnorm_v38_scalar_kernel<scalar_t><<<batch_size, block_size, smem_size>>>(
                        input.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(),
                        weight.data_ptr<scalar_t>(), bias.data_ptr<scalar_t>(),
