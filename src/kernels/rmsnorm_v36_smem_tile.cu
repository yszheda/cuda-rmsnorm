...
// - Loads entire row into shared memory
// - Computes sum-of-squares and normalize from smem
// - Eliminates redundant global memory reads
// - Only for hidden_dim <= 4096 (fits in 16KB smem for fp16)
// ============================================================================

template<typename T, int vec_width>
__global__ void rmsnorm_v36_smem_tile_kernel(
    ...
    // Shared memory for the entire row
    ...
    for (int64_t i = threadIdx.x; i < hidden_dim; i += tile_size) {
        if (i < hidden_dim) {
    ...
    float sum_sq = 0.0f;
    for (int64_t i = threadIdx.x; i < num_vec; i += tile_size) {
        float4 v;
        const float4* input_vec = reinterpret_cast<const float4*>(smem_input);
        v = input_vec[i];
    ...
    for (int64_t i = vec_dim + threadIdx.x; i < hidden_dim; i += tile_size) {
        float x = ConvertOps<T>::to(smem_input[i]);
        sum_sq += x * x;
    }

    float total = block_reduce_sum(sum_sq, smem_reduce, tile_size);
    float rms = rsqrtf(total / hidden_dim + eps);

    // Normalize from shared memory, write to global
...
// Scalar fallback
template<typename T>
__global__ void rmsnorm_v36_scalar_kernel(
    const T* __restrict__ input,
    T* __restrict__ output,
    const T* __restrict__ weight,
    const T* __restrict__ bias,
...
void rmsnorm_v36_smem_tile_cuda(
    ...
    // Shared memory size: input (hidden_dim elements) + reduction (64 floats)
    const int smem_cap = 4096;
    bool use_tile = hidden_dim <= smem_cap;
    size_t smem_size = 64 * sizeof(float);
    if (use_tile) {
        smem_size += hidden_dim * sizeof(float);  // input in smem
    }

    AT_DISPATCH_FLOATING_TYPES_AND2(
        at::ScalarType::Half, at::ScalarType::BFloat16,
        input.scalar_type(), "rmsnorm_v36_smem_tile",
        [&]() {
            ...
            if (aligned && use_tile) {
                rmsnorm_v36_smem_tile_kernel<scalar_t, vw><<<batch_size, block_size, smem_size>>>(
                ...
                rmsnorm_v36_scalar_kernel<scalar_t><<<batch_size, block_size, smem_size>>>(
                    input.data_ptr<scalar_t>(),
                    output.data_ptr<scalar_t>(),
                    weight.data_ptr<scalar_t>(),
                    bias.data_ptr<scalar_t>(),
