#!/usr/bin/env python3
"""CUDA RMSNorm Final Comprehensive Report with All Kernels"""
import torch
import sys
import numpy as np
...
print("=" * 140)
print("CUDA RMSNorm Final Comprehensive Performance Report")
print("GPU: NVIDIA Thor (compute cap 11.0, peak BW ~800 GB/s)")
print("=" * 140)
print()

print("1. Full Kernel Comparison (min of 3 runs)")
print("-" * 140)
...
print()
print("2. Performance Bar Chart (v13 vs v15 vs v35, normalized to best=1.0)")
print("-" * 140)

for shape, dtype, name in shapes:
    times = results[(name, dtype)]
    best_t = min(times[v] for v in [13, 15, 35])
    ...
    for v in [13, 15, 35]:
        ...
        bar = "#" * bar_len
        star = "*" if v == min([v2 for v2 in [13, 15, 35] if times[v2] == best_t]) else " "
        print("  v{:2d} {:>7.3f}x |{} {}".format(v, ratio, bar, star))

# 3. Bandwidth utilization
print()
print("3. Effective Bandwidth (peak ~800 GB/s)")
print("-" * 140)
...
for shape, dtype, name in shapes:
    times = results[(name, dtype)]
    best_t = min(times[v] for v in kernels)
    v13_t = times[13]
    ratio = v13_t / best_t
